...
const ApplePayIcon = () => (
  <svg fill="#000000" width="45" height="45" viewBox="0 -34.55 120.3 120.3" xmlns="http://www.w3.org/2000/svg">
...
const paymentOptions = [
  ...
  },
  {
    id: "APAY",
    title: "Apple Pay ",
    description: "Pagamento rapido con wallet digitale",
    icon: <ApplePayIcon />,
  },
  {
    id: "GPAY",
...
export default function Pagamento() {
  ...
  async function handleContinue() {
    ...
    const pagamentoData = {
      pagamento_metodo: selectedMethod,
      veicolo_id: veicolo_id,
      prenotazione_id: prenotazione_id,
      cliente_id: userId,
      pagamento_status: "in attesa",
    };
    const response = await supabase.from("pagamenti").insert(pagamentoData);

    if (response.error) {
      alert("Errore durante la conferma del pagamento. Riprova.");
      return;
    }

    navigate("/chekout", { state: { prezzo_giornaliero, prenotazione_id, userId ,veicolo_id,selectedMethod } });
  }
    ...
    <div className="pagamento-container">
      <div className="pagamento-content">
        ...
        <div className="pagamento-header">
          ...
          <h1 className="pagamento-title">Seleziona il metodo di pagamento</h1>
        </div>
        ...
        <div className="payment-methods">
          {paymentOptions.map((option) => (
            <div
              key={option.id}
              className={`payment-card ${selectedMethod === option.id ? "selected" : ""}`}
              onClick={() => setSelectedMethod(option.id)}
            >
              <div className="payment-card-content">
                {/* Icon */}
                <div className={`payment-icon ${selectedMethod === option.id ? "active" : ""}`}>
                ...
                {/* Check indicator */}
                <div className={`check-indicator ${selectedMethod === option.id ? "checked" : ""}`}>
                  {selectedMethod === option.id && <Check size={16} color="white" />}
                </div>
              </div>
            </div>
          ))}
        ...
        {/* Continue Button */}
        <button
          className={`continue-button ${selectedMethod ? "active" : "disabled"}`}
